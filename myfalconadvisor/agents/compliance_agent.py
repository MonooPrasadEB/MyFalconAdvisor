...
class AuditLogger:
    ...
    def __init__(self, log_path: Optional[str]="compliance.log"):
    ...
    @classmethod
    def get(cls): 
        if not cls._instance: cls._instance=AuditLogger()
        return cls._instance
    def connect_db(self, db_config: Optional[Dict[str, str]] = None):
        """Connect to PostgreSQL database for audit logging."""
        if db_config:
            self.db_config = db_config
        else:
            # Use default database configuration from environment
        ...
        try:
            ...
            # Verify the existing compliance_checks table exists
            with self.db.cursor() as cursor:
                ...
                if not table_exists:
                    self.logger.error("compliance_checks table does not exist in the database")
                    self.db = None
                    return
                
            self.logger.info("Connected to PostgreSQL audit database using existing compliance_checks table")
        except Exception as e:
            self.logger.error(f"Failed to connect to PostgreSQL audit database: {e}")
            self.db = None
    def _emit(self, payload: Dict[str,Any]):
        ...
        try:
            with self.db.cursor() as cursor:
                if payload.get("event")=="compliance_event":
                    # Map to existing compliance_checks table structure
                    result_obj = payload.get("result", {})
                    violations = result_obj.get("violations", [])
                    
                    # Determine the primary rule and violation details
                    primary_rule = violations[0] if violations else None
                    rule_name = primary_rule.get("rule_id", "GENERAL") if primary_rule else "GENERAL"
                    rule_description = primary_rule.get("description", "") if primary_rule else ""
                    
                    # Map severity to allowed values: low, medium, high, critical
                    severity_map = {
                        "advisory": "low",
                        "warning": "medium", 
                        "major": "high",
                    ...
                    severity = severity_map.get(primary_rule.get("severity", "warning"), "medium") if primary_rule else "low"
                    
                    # Map check_result to allowed values: pass, fail, warning
                    decision = payload["decision"]  # approved/rejected
                    check_result = "fail" if decision == "rejected" else ("warning" if violations else "pass")
                    
                    # Map check_type to allowed values: suitability, concentration, liquidity, regulatory, risk_limit
                    check_type_map = {
                        "trade": "regulatory",
                        "portfolio": "concentration", 
                        "suitability": "suitability"
                    }
                    mapped_check_type = check_type_map.get(payload["type"], "regulatory")
                    
                    # Create violation details JSON
                    violation_details = {
                        "violations": violations,
                        "warnings": result_obj.get("warnings", []),
                        "recommendations": result_obj.get("recommendations", []),
                    ...
                    cursor.execute("""
                        INSERT INTO compliance_checks(
                            check_id, check_type, rule_name, rule_description, 
                            check_result, violation_details, severity, 
                            checked_at, auto_resolved
                        ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)
                    """, (
                        payload["id"],  # check_id (UUID)
                        mapped_check_type,  # check_type (mapped to allowed values)
                        rule_name,  # rule_name
                        rule_description,  # rule_description
                        check_result,  # check_result (pass/fail/warning)
                        json.dumps(violation_details),  # violation_details (JSONB)
                        severity,  # severity (mapped to allowed values)
                        payload["at"],  # checked_at
                        False  # auto_resolved
                    ))
                    
        except Exception as e:
            self.logger.error(f"Failed to write audit log to PostgreSQL: {e}")
...
class PolicyStore:
    ...
    def _log_policy_change(self, old, new):
        try:
            ...
            old_d={"version":old.version,"rules":{k:_dataclass_to_dict(v) for k,v in old.rules.items()}}
            new_d={"version":new.version,"rules":{k:_dataclass_to_dict(v) for k,v in new.rules.items()}}
...
if __name__=="__main__":
    ...
    AuditLogger.get().connect_db()  # Connect to PostgreSQL using default config
